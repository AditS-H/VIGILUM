...
func (fe *DefaultFeatureExtractor) calculateRiskIndicators(source string, bytecode []byte) float64 {
	...
	if strings.Contains(source, "block.timestamp") && strings.Contains(source, "require") == false {
		riskScore += 0.15
	}
	if strings.Contains(source, ".transfer(") && strings.Contains(source, "require") == false {
		riskScore += 0.1
	}
